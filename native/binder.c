...
static struct binder_thread *binder_get_thread(struct binder_proc *proc)
{
	...
	// 查找pid 等于当前线程id的thread，并且红黑树以pid 大小为序存放
	while (*p) {
		parent = *p;
...
{
	int ret;
	// 另外注意 binder_open() 方法处理两个工作
	// 创建并初始化每一个进程独一份用来存放binder相关数据的binder_proc
	// 将binder_proc记录起来，以后用
	...
	case BINDER_WRITE_READ: {
		...
		if (bwr.write_size > 0) { // 写数据
			// 处理binder数据读写， binder IPC 通信的核心逻辑
